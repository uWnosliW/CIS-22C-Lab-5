...
public:
    ...
    // Return: ostream
    friend std::ostream & operator<<(std::ostream &out, const Rupee& c);
    ...
    // Post: formatted currency into the buffer
    // Return: istream
    friend std::istream & operator>>(std::istream &in, Rupee& c);
...
protected:
private:
    std::string unitName = "Rupee";
    std::string fracName = "paise";
};
